...
let
  ...
  stalwartPkg = pkgs.stalwart.overrideAttrs (oldAttrs: {
    buildFeatures = (oldAttrs.buildFeatures or [ ]) ++ [ "zenoh" ];
  });
  ...
  stalwartConfig = {
      ...
      config = ''
        {
          ...
          connect: {
            endpoints: [
              ${if cfg.nodeId == 1 then ''"tcp/${rekkakenIp}:17447"'' else ''"tcp/${shoryukenIp}:17447"''}
            ],
          },
          listen: {
            endpoints: [
    ...
    # Queue configuration - Outbound TLS settings
    queue = {
      retry = [
        "2m"
        "5m"
    ...
    # Require STARTTLS for all outgoing mail (no unencrypted delivery)
    "queue.tls".default = {
      starttls = "require";
    ...
    # Auto-ban configuration - aggressive security
    ...
    # Web admin
    webadmin = {
      path = lib.mkForce "/var/cache/stalwart";
      resource = lib.mkForce "file://${stalwartPkg.webadmin}/webadmin.zip";
    };
