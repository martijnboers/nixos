...
in {
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services."adguard-exporter-prometheus" = {
      ...
      serviceConfig = {
        ExecStart = "${pkgs.adguard-exporter}/bin/adguard-exporter -adguard_hostname 127.0.0.1 -adguard_port ${toString config.adguardhome.settings.bind_port} -log_limit 10000";
        Restart = "on-failure";
        RestartSec = 5;
